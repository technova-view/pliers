...
export class AuthService {
  ...
  async signup(signupDto: SignupDto): Promise<BaseApiResponse<{ userId: string }>> {
    ...
    const user = this.userRepository.create({
      ...
      provider: 'email',
      userType,
      status: userType === UserType.CONTRACTOR ? UserStatus.PENDING : UserStatus.ACTIVE,
    });
  ...
  async login(loginDto: LoginDto): Promise<BaseApiResponse<AuthTokensResponseDto>> {
    ...
    if (user.status !== UserStatus.ACTIVE)
      throw new UnauthorizedException('You can\'t login without an admin activating your account');
  ...
  async googleAuth(googleAuthDto: GoogleAuthDto): Promise<BaseApiResponse<AuthTokensResponseDto>> {
    ...
    try {
      ...
      if (user.status !== UserStatus.ACTIVE)
        throw new BadRequestException('You can\'t login without an admin activating your account');
